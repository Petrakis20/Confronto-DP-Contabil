...
def main():
    ...
    # Inicializar contador de upload (para renovar keys e limpar arquivos)
    if 'upload_counter' not in st.session_state:
        st.session_state['upload_counter'] = 0

    # Seção de Manuais (sempre visível)
    st.header("📚 Manuais do Sistema")
    st.markdown("Baixe os manuais para aprender a usar o sistema:")
    
    # Botões de download dos manuais
    col_manual1, col_manual2 = st.columns(2)
    
    with col_manual1:
        # Carregar Manual da Plataforma
        manual_plataforma_path = Path(__file__).parent / "Manual Plataforma Input DP.pdf"
        if manual_plataforma_path.exists():
            with open(manual_plataforma_path, 'rb') as f:
                manual_plataforma_bytes = f.read()
            
            st.download_button(
                label="📖 Manual da Plataforma",
                data=manual_plataforma_bytes,
                file_name="Manual_Plataforma_Input_DP.pdf",
                mime="application/pdf",
                use_container_width=True,
                help="Guia completo de uso da plataforma"
            )
        else:
            st.warning("Manual da Plataforma não encontrado")
    
    with col_manual2:
        # Carregar Manual de Extração do Lote
        manual_lote_path = Path(__file__).parent / "Passo a Passo txt do Lote.pdf"
        if manual_lote_path.exists():
            with open(manual_lote_path, 'rb') as f:
                manual_lote_bytes = f.read()
            
            st.download_button(
                label="📋 Passo a Passo - Extração do Lote",
                data=manual_lote_bytes,
                file_name="Passo_a_Passo_txt_do_Lote.pdf",
                mime="application/pdf",
                use_container_width=True,
                help="Guia para extrair o arquivo TXT do lote"
            )
        else:
            st.warning("Manual de Extração do Lote não encontrado")
    
    st.markdown("---")
    
    # Seção de upload de arquivos (SEMPRE VISÍVEL)
    st.header("📁 Upload de Arquivos")
    
    col1, col2 = st.columns(2)
    
    with col1:
        # Upload de PDFs com key dinâmica
    ...
    # Se não há arquivos, mostrar mensagem
    if not pdf_files and not st.session_state.get('processado', False):
        st.info("📌 Faça upload dos arquivos PDF acima para começar")
        return
    ...
    if st.session_state.get('processado', False):
        # Botão "Fazer Novo Confronto" no topo
        st.markdown("---")
        col_novo1, col_novo2, col_novo3 = st.columns([1, 1, 1])
        with col_novo2:
            if st.button("🔄 Fazer Novo Confronto", type="secondary", use_container_width=True, key="novo_confronto_top"):
                # Incrementar contador para renovar keys dos uploaders
                st.session_state['upload_counter'] += 1
                # Limpar todos os outros dados do session_state (exceto o contador)
                keys_to_delete = [key for key in st.session_state.keys() if key != 'upload_counter']
            ...
            try:
                ...
                if not df_liquidos.empty:
                    ...
                    for _, row in df_liquidos.iterrows():
                        ...
                        # Cor padrão para todos os blocos de líquidos: RGB(25, 68, 126)
                        cor_padrao = 'rgb(25, 68, 126)'
                        ...
                        with col4:
                            st.markdown(f"""
                            <div style="background: {cor_padrao};
        ...
        if impostos_geral:
            ...
            # Cards principais em 4 colunas (INSS, FGTS, IRRF, Empréstimo)
            col1, col2, col3, col4 = st.columns(4)

            with col1:
                st.markdown(f"""
                <div style="background: rgb(25, 68, 126);
            ...
            # Quarta coluna - Empréstimo (se houver dados de TXT)
            with col4:
                if 'emprestimos_fgts' in st.session_state:
                    ...
                    <div style="background: rgb(25, 68, 126);
                                padding: 20px; border-radius: 10px; text-align: center;
                                box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <h4 style="color: white; margin: 0;">💳 Empréstimo</h4>
                        <p style="color: white; font-size: 1.8em; font-weight: bold; margin: 10px 0;">
                    ...
                    # Placeholder vazio se não houver dados de empréstimo
                    st.markdown("")
...
Diferença = {money(diferenca_autonomos)}
        ...
        # Botão "Fazer Novo Confronto" no final
        st.markdown("---")
        col_final1, col_final2, col_final3 = st.columns([1, 1, 1])
        with col_final2:
            if st.button("🔄 Fazer Novo Confronto", type="secondary", use_container_width=True, key="novo_confronto_bottom"):
                # Incrementar contador para renovar keys dos uploaders
                st.session_state['upload_counter'] += 1
                # Limpar todos os outros dados do session_state (exceto o contador)
                keys_to_delete = [key for key in st.session_state.keys() if key != 'upload_counter']
